...
///
/// Is not triggered when the [`ColliderConstructor`] failed to construct the [`Collider`] or when there was already a [`Collider`] on the entity.
...
///
/// Note that the event will still be triggered when when the hierarchy had no colliders to insert or failed to insert all of them,
/// so this event is not a guarantee that there are actually any colliders in the scene.
#[derive(EntityEvent, Clone, Copy, Debug, PartialEq)]
#[cfg_attr(feature = "serialize", derive(serde::Serialize, serde::Deserialize))]
