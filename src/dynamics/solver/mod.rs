...
mod penetration;
pub use penetration::PenetrationConstraint;
use xpbd::{PositionConstraint, XpbdConstraint};
                ...
                for contact in manifold.contacts.iter() {
                    ...
                    let contact = ContactData {
                        point1: collider1.transform.map_or(contact.point1, |t| {
                            t.rotation * contact.point1 + t.translation
                        }),
                        point2: collider2.transform.map_or(contact.point2, |t| {
                            t.rotation * contact.point2 + t.translation
                        }),
                        normal1: collider1
                            .transform
                            .map_or(contact.normal1, |t| t.rotation * contact.normal1),
                        normal2: collider2
                            .transform
                            .map_or(contact.normal2, |t| t.rotation * contact.normal2),
                        ..*contact
                    };
...
fn update_ang_vel(
    ...
    for (rb, rot, prev_rot, mut ang_vel, mut pre_solve_ang_vel) in &mut bodies {
        ...
        if rb.is_dynamic() {
            let new_ang_vel = (*rot * prev_rot.inverse()).as_radians() / delta_secs;
            // avoid triggering bevy's change detection unnecessarily
...
fn solve_vel(
    ...
    for constraint in penetration_constraints.0.iter_mut() {
        if let Ok([mut body1, mut body2]) = bodies.get_many_mut(constraint.entities()) {
            ...
            let r1 = *body1.rotation * constraint.r1;
            let r2 = *body2.rotation * constraint.r2;
