...
/// 4. Prepare contact constraints ([`SolverSet::PrepareContactConstraints`])
/// 5. Substepping loop (runs the [`SubstepSchedule`] [`SubstepCount`] times; see [`SolverSet::Substep`])
/// 6. Apply restitution ([`SolverSet::Restitution`])
/// 7. Write back solver body data to rigid bodies. ([`SolverSet::Finalize`])
/// 8. Store contact impulses for next frame's warm starting ([`SolverSet::StoreContactImpulses`])
