...
impl<'w> RigidBodyQueryItem<'w> {
    ...
    pub fn angular_inertia(&self) -> RigidBodyAngularInertia {
        if self.rb.is_dynamic() {
            *self.angular_inertia
        } else {
            RigidBodyAngularInertia::INFINITY
        }
    }

    /// Computes the effective world-space angular inertia, taking into account any rotation locking.
    pub fn effective_global_angular_inertia(&self) -> RigidBodyAngularInertia {
        if !self.rb.is_dynamic() {
            return RigidBodyAngularInertia::INFINITY;
        }
...
impl<'w> RigidBodyQueryReadOnlyItem<'w> {
    ...
    pub fn mass(&self) -> RigidBodyMass {
        if self.rb.is_dynamic() {
            *self.mass
        } else {
            RigidBodyMass::INFINITY
        }
    }
