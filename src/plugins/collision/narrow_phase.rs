...
impl Plugin for NarrowPhasePlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<NarrowPhaseConfig2d>()
            .init_resource::<NarrowPhaseConfig3d>()
            .init_resource::<Collisions2d>()
            .init_resource::<Collisions3d>()
            .register_type::<NarrowPhaseConfig2d>()
            .register_type::<NarrowPhaseConfig3d>();
        ...
        app.get_schedule_mut(PhysicsSchedule)
            ...
            .add_systems(
                (
                    ...
                    (reset_collision_states_2d, reset_collision_states_3d)
        ...
        app.get_schedule_mut(SubstepSchedule)
            ...
            .add_systems(
                (
                    (
                        reset_substep_collision_states_2d,
                        reset_substep_collision_states_3d,
                    ),
                    (collect_collisions_2d, collect_collisions_3d),
                )
                    .chain()
...
#[reflect(Resource)]
pub struct NarrowPhaseConfig2d {
...
impl Default for NarrowPhaseConfig2d {
    fn default() -> Self {
        Self {
...
#[derive(Resource, Reflect, Clone, Debug, PartialEq)]
#[reflect(Resource)]
pub struct NarrowPhaseConfig3d {
    /// The maximum separation distance allowed for a collision to be accepted.
    ///
    /// This can be used for things like **speculative contacts** where the contacts should
    /// include pairs of entities that *might* be in contact after constraint solving or
...
impl Default for NarrowPhaseConfig3d {
    fn default() -> Self {
        Self {
...
pub fn collect_collisions_2d(
    ...
    broad_collision_pairs: Res<BroadCollisionPairs2d>,
    mut collisions: ResMut<Collisions2d>,
    narrow_phase_config: Res<NarrowPhaseConfig2d>,
) {
    #[cfg(feature = "parallel")]
    {
        ...
        let new_collisions = broad_collision_pairs
            ...
            .par_splat_map(pool, None, |chunks| {
                let mut new_collisions: Vec<Contacts2d> = vec![];
                for (entity1, entity2) in chunks {
                    if let Ok([bundle1, bundle2]) = bodies.get_many([*entity1, *entity2]) {
                        ...
                        let contacts = Contacts2d {
                            ...
                            manifolds: contact_query::contact_manifolds_2d(
                                collider1,
                                position1,
                                *rotation1,
                                collider2,
...
pub fn collect_collisions_3d(
    ...
    broad_collision_pairs: Res<BroadCollisionPairs3d>,
    mut collisions: ResMut<Collisions3d>,
    narrow_phase_config: Res<NarrowPhaseConfig3d>,
) {
    #[cfg(feature = "parallel")]
    {
        let pool = ComputeTaskPool::get();
        ...
        let new_collisions = broad_collision_pairs
            .0
            .par_splat_map(pool, None, |chunks| {
                let mut new_collisions: Vec<Contacts3d> = vec![];
                for (entity1, entity2) in chunks {
                    if let Ok([bundle1, bundle2]) = bodies.get_many([*entity1, *entity2]) {
                        let (position1, accumulated_translation1, rotation1, collider1) = bundle1;
                        let (position2, accumulated_translation2, rotation2, collider2) = bundle2;
                        ...
                        let contacts = Contacts3d {
                            ...
                            during_previous_frame: previous_contact
                                .map_or(false, |c| c.during_previous_frame),
                            manifolds: contact_query::contact_manifolds_3d(
                                collider1,
                                position1,
                                *rotation1,
                                collider2,
...
pub fn reset_substep_collision_states_2d(mut collisions: ResMut<Collisions2d>) {
    for contacts in collisions.get_internal_mut().values_mut() {
        contacts.during_current_substep = false;
    }
}
...
/// Reset `during_current_substep` for each collision in [`Collisions`].
pub fn reset_substep_collision_states_3d(mut collisions: ResMut<Collisions3d>) {
    for contacts in collisions.get_internal_mut().values_mut() {
        contacts.during_current_substep = false;
    }
}
